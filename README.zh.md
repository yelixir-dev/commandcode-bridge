...
当前 bridge version：**v0.32.1**。

版本也会从 `/health` 返回，并显示在 web dashboard 右上角。

### v0.32.1 CommandCode 兼容性更新

此 bridge release 已对齐官方 `command-code` npm package `0.32.1`：

- 默认上游 `x-command-code-version` header 现在发送 `0.32.1`，除非用 `COMMANDCODE_CLI_VERSION` 覆盖。
- bridge package/runtime version 也是 `0.32.1`，因此 `/health`、dashboard 和 npm metadata 与目标 CommandCode CLI version 对齐。
- 直接检查 `command-code@0.32.1` bundle 后确认，bridge-critical API paths 仍兼容：`/alpha/generate`、`/alpha/whoami`、`/alpha/billing/credits`、`/alpha/billing/subscriptions`、`/alpha/usage/summary`。
- model catalog 已从 `0.32.1` CLI bundle 刷新。现有 enabled defaults 保持保守；额外发现的 Qwen 3.6 Max Preview、MiniMax M3、MiniMax M2.5、Kimi K2.5、Step 3.5 Flash、Gemini 3.5 Flash、Gemini 3.1 Flash Lite、GLM-5、GPT 5.4/5.3 Codex/5.4 Mini、Claude Opus 4.8 和旧 Claude variants 等模型会保留为 disabled，直到 operator 主动启用。
...
| `COMMANDCODE_CLI_VERSION`                   | `0.32.1`                     | 发送给 upstream 的 version header。                                                                              |
