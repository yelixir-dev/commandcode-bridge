...
현재 bridge version: **v0.30.1**.

버전은 `/health` 응답과 웹 대시보드 오른쪽 위에 표시됩니다.

### v0.30.1 CommandCode 호환성 업데이트

이번 bridge release는 공식 `command-code` npm package `0.30.1`에 맞췄습니다.

- 기본 upstream `x-command-code-version` header는 `COMMANDCODE_CLI_VERSION`으로 덮어쓰지 않는 한 `0.30.1`을 보냅니다.
- bridge package/runtime version도 `0.30.1`이라 `/health`, dashboard, npm metadata가 대상 CommandCode CLI version과 일치합니다.
- `command-code@0.30.1` bundle을 직접 확인한 결과 bridge 핵심 API path(`/alpha/generate`, `/alpha/whoami`, `/alpha/billing/credits`, `/alpha/billing/subscriptions`, `/alpha/usage/summary`)는 기존 bridge 경로와 호환됩니다.
- model catalog를 `0.30.1` CLI bundle 기준으로 갱신했습니다. 기존 enabled default는 보수적으로 유지하고, Qwen 3.6 Max Preview, MiniMax M2.5, Kimi K2.5, Step 3.5 Flash, Gemini 3.5 Flash, Gemini 3.1 Flash Lite, GLM-5, GPT 5.4/5.3 Codex/5.4 Mini, Claude Opus 4.8, 구 Claude variant 등 추가 발견 모델은 operator가 켤 때까지 disabled 상태로 포함했습니다.
...
| `COMMANDCODE_CLI_VERSION`                    | `0.30.1`                     | upstream으로 보내는 version header.                                                                        |
